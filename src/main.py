...
class ProcessConvert(QThread):
    ...
    def run(self):
        ...
        n = len(files)
        convert = Convert(self.params["target"])
        num_parts = 0
        ...
        for i, filepath in enumerate(files):
            if i == 0:
                ...
                self.count_changed.emit((count, i, f"{old_file} split to {num_parts} part", filepath, 30000))
            time.sleep(1)
            count = (i+1)/n * 100
            print((i+1), n, count)
            try:
                num_parts = convert.split_file(filepath)
                old_file = filepath.replace("/", "\\").split("\\")[-1]
            except AssertionError:
                num_parts = f"Error"
        self.count_changed.emit((count, i, f"{old_file} split to {num_parts} part", "", 30000))
        time.sleep(5)
        self.count_changed.emit((100, n, f"Process took ~{(time.time()-self.params['start_t0'])/60:.1f} (min)!", "", 100000))
...
class Ui_MainWindow2(Ui_MainWindow):
    ...
    def show_msg_box(self, title="MessageBox demo", msg_text="This is a message box", sub_msg="This is additional information", detail=None):
